...
@section('page-actions')
    <a href="{{ route('backend.lapangan.create') }}"
        class="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg inline-flex items-center transition-colors duration-200">
                ...
                <tbody class="text-gray-300">
                    @forelse($lapangans as $lapangan)
                        <tr class="border-b border-white/10 hover:bg-white/5 transition-colors duration-200">
                            <td class="py-3 px-4">
                                @if ($lapangan->foto)
                                    <img src="{{ Storage::url($lapangan->foto) }}" alt="{{ $lapangan->nama_lapangan }}"
                                        class="w-16 h-12 object-cover rounded-md">
                                @else
                                    <div
                                        class="w-16 h-12 bg-gray-700 flex items-center justify-center text-xs text-gray-400 rounded-md">
                                        No Image</div>
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-4 text-right">
                                <div class="flex justify-end items-center space-x-4">
                                    {{-- Tombol Edit --}}
                                    <a href="{{ route('backend.lapangan.edit', $lapangan) }}"
