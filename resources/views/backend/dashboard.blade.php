...
@section('page-actions')
    ...
    <div class="flex items-center space-x-2">
        <form action="{{ route('backend.dashboard') }}" method="GET">
            <input type="date" id="dateFilter" name="date" class="bg-white/10 border-white/20 rounded-md shadow-sm text-white" value="{{ $selectedDate }}" onchange="this.form.submit()">
        </form>
                        ...
                        <td class="p-1">
                            @if($booking)
                                <div class="bg-red-500/30 text-red-100 p-2 rounded-md text-center h-full">
                                    ...
                                    <p class="text-xs">{{ \Carbon\Carbon::parse($booking->jam_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($booking->jam_selesai)->format('H:i') }}</p>
                                </div>
                            @else
